...
1. Backend สุ่มตัวอย่างเนื้อหาจาก CSV (ไม่ต้องส่งทั้งไฟล์ถ้าไฟล์ใหญ่มาก — สุ่ม/สรุปตัวแทนพอ)
2. ส่ง prompt ให้ Gemini วิเคราะห์และคืนค่าเป็น JSON structured output เช่น:
   ```json
   {
     "glossary": [
   ...
   ```
3. บันทึกผลลง `glossary_entries` (created_by = "ai")
4. ตั้ง job.status = `awaiting_review` แล้วรอผู้ใช้กดยืนยันหน้า UI
